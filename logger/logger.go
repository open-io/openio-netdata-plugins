...
func NewCollector(path string) *collector {
    targets, err := ParseLogConfig(path)
    if err != nil {
...
func ParseLogConfig(path string) (targets map[string]*LogTarget, err error) {
    ...
    filtered, err := yaml.Marshal(func() map[string]interface{} {
        ...
        for k, v := range(cast) {
            if _, ok := v.(string); ok { continue }
            if _, ok := v.(int64); ok { continue }
            if _, ok := v.(float64); ok { continue }
            res[k] = v
        }
        return res
    }())
...
// netdata_web_log_detailed_response_codes_requests_persec_average
// netdata_web_log_bandwidth_kilobits_persec_average
// netdata_web_log_response_time_milliseconds_average



func (c *collector) Collect() (map[string]string, error) {
    // RESET ALL COUNTERS
    // c.metrics.Requests = map[string][]int64{}
    // c.metrics.Latency = map[string][]float64{}
    // c.metrics.BandwidthIn = map[string][]int64{}
    // c.metrics.BandwidthOut = map[string][]int64{}
...
func (c *collector) tailLogFile(name string, conf *LogTarget) {
    ...
    for {
        if _, err := os.Stat(conf.Path); err != nil {
            log.Println("WARN: Could not watch file", conf.Path, ":", err, ". Retrying in", retry, "seconds")
        ...
        for line := range t.Lines {
            ...
            // fmt.Println("Reqs", c.metrics.Requests)
            // fmt.Println("PerfData", c.metrics.PerfData)
        }
    }
